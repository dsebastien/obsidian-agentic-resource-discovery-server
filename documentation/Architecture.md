...
| `search/`  | `search-backend` (interface), `lexical-search-backend` (MiniSearch), `semantic-search-backend` (hybrid), `vector-store`, `rrf`, `embedding/embedder` + `embedding/http-embedder`, `search-backend-factory` | Pluggable relevance ranking; 0–100 score. Lexical BM25 default; `local-model` fuses BM25 + dense-vector cosine (RRF), embeddings from a local OpenAI-compatible server behind an injectable `Embedder` seam.                                                                                                   |
...
- **`SearchBackend`** — `index(entries)` / `search(req)` / `isReady()`. Lexical (default) and the hybrid `SemanticSearchBackend` ship today; qmd/hosted are drop-ins (`search-backend-factory`).
- **`Embedder`** — `load()` / `embed(texts)` / `isReady()`, returns L2-normalised vectors. Injectable: a deterministic fake drives `SemanticSearchBackend`'s unit tests; `HttpEmbedder` calls a local OpenAI-compatible embedding server (Ollama, LM Studio, …) via `requestUrl` — nothing bundled or downloaded, and an unreachable server degrades to lexical.
