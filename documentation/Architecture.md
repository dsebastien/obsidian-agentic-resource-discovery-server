...
| `search/`  | `search-backend` (interface), `lexical-search-backend` (MiniSearch), `semantic-search-backend` (hybrid), `vector-store`, `rrf`, `embedding/embedder` + `embedding/transformers-embedder`, `search-backend-factory` | Pluggable relevance ranking; 0–100 score. Lexical BM25 default; `local-model` fuses BM25 + dense-vector cosine (RRF), embeddings behind an injectable `Embedder` seam.                                                                                                   |
...
- **`Embedder`** — `load()` / `embed(texts)` / `isReady()`, returns L2-normalised vectors. Injectable: a deterministic fake drives `SemanticSearchBackend`'s unit tests; `TransformersEmbedder` lazy-loads an ONNX model at runtime (unbundled until shipped, so it degrades to lexical).
