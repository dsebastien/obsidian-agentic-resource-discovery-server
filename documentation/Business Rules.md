...
- **BR-4 — Confined file serving.** Skill files are served only from configured skill roots, only for allowlisted extensions, and only via `safeJoin` (rejects `..`, URL-encoded traversal, and null bytes). The resolved path is additionally `realpath`-checked so a symlink inside a skill folder cannot escape the root. Artifact bodies (a skill's `SKILL.md`, a subagent definition) resolve **by URN through the artifact store, never by parsing a URL**, and are re-checked against their scan root at serve time; an entry whose `url` merely looks like a local route gets no body.
...
- **BR-16 — Subagent family is additive and opt-in.** One definition file → one entry; only that file is servable (no bundled assets). `agentFolders` defaults to empty: a default would publish every definition's name, description and tool list through the public catalog on upgrade. Both families land in one scan snapshot and one rebuild; a failing scan of either leaves the previous catalog intact.
