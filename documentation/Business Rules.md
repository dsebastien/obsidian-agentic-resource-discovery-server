...
- **BR-4 — Confined file serving.** Skill files are served only from configured skill roots, only for allowlisted extensions, and only via `safeJoin` (rejects `..`, URL-encoded traversal, and null bytes). The resolved path is additionally `realpath`-checked so a symlink inside a skill folder cannot escape the root.
