...
| --------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Semantic search backends**                        | `search/search-backend-factory.ts` (+ new `*-search-backend.ts`)                                                  | Add `LocalModelSearchBackend` (Transformers.js ONNX, ~23 MB lazy download), `QMDSidecarSearchBackend`, `HostedEmbeddingSearchBackend`. Each is a drop-in `SearchBackend`; fuse scores into 0–100 (RRF). Recipe + rationale in §8.3–8.6. **Honor the no-mandatory-download non-goal** — keep lexical the default. After switching backend, the **Reindex** button rebuilds the index in place. |
