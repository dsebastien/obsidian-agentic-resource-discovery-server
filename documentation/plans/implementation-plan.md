...
Everything M0–M6 is implemented, tested (166 specs), documented, and on `origin/main`. The plugin runs end-to-end and was verified against the real 395-skill vault and the official MCP SDK client. What's left is **post-v1, all optional**, none blocking use:

| Follow-up                                           | Where                                                                                                             | Notes / recipe                                                                                                                                                                                                                                                                                                                           |
| --------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Semantic search backends**                        | `search/search-backend-factory.ts` (+ new `*-search-backend.ts`)                                                  | Add `LocalModelSearchBackend` (Transformers.js ONNX, ~23 MB lazy download), `QMDSidecarSearchBackend`, `HostedEmbeddingSearchBackend`. Each is a drop-in `SearchBackend`; fuse scores into 0–100 (RRF). Recipe + rationale in §8.3–8.6. **Honor the no-mandatory-download non-goal** — keep lexical the default.                         |
| **"Reindex" button in the Search backend settings** | `settings/settings-tab.ts` (`renderSearchBackendSection`) + a `RegistryController.reindex()` / `plugin.reindex()` | Add a button to the **Search backend** section that re-runs `SearchBackend.index(...)` over the current catalog (and, for future model-backed backends, regenerates embeddings) without a full rescan. Useful after switching backend or to refresh the index. Show a transient "Reindexing…" state like the "Rescan skills now" button. |
| **Per-IP rate limiting**                            | `server/router.ts` (or a small middleware in `registry-controller`)                                               | Token-bucket, ~100 req/min; return 429.                                                                                                                                                                                                                                                                                                  |
