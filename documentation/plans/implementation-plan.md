...
| **qmd sidecar backend** | `search/search-backend-factory.ts` (+ new `qmd-*.ts`) | `QMDSidecarSearchBackend` — spawn/manage the `qmd` CLI as a sidecar, fuse via `rrf.ts`. Recipe in §8.4. Only remaining backend; **honor the no-mandatory-download non-goal** — keep lexical the default. |
