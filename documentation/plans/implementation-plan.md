...
| **M5** — Pluggable backend factory                     | ✅ Done (2026-06-23) | `search/search-backend-factory` (`createSearchBackend`) wired into `RegistryController`; a backend-kind change restarts the registry. **Hybrid semantic core now built & tested** (post-v1): `local-model` → `SemanticSearchBackend` (lexical BM25 ⊕ dense-vector cosine via RRF) behind an injectable `Embedder` seam (`VectorStore`, `rrf.ts`, real `TransformersEmbedder` lazy-imported & unbundled). Two-phase indexing — lexical serves instantly, embeddings build in the background; degrades to lexical until/unless the model loads. qmd/hosted backends still deferred. **Remaining: bundle `@huggingface/transformers` + verify in a live vault** (see §8.3 status + §1b). 189 tests.                                                                                                          |
...
| --------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Semantic search — bundle + verify the model** | `search/embedding/transformers-embedder.ts`, `scripts/build.ts` | **Hybrid core is built & tested** (see §8.3 status): `local-model` now routes to `SemanticSearchBackend` (lexical BM25 ⊕ dense-vector cosine via RRF) behind an injectable `Embedder` seam, with `VectorStore`, `rrf.ts`, and a real `TransformersEmbedder`. The embedder lazy-imports `@huggingface/transformers` via a **runtime-computed specifier**, so it is *not* bundled today — `local-model` degrades to lexical until the lib ships. Remaining: add `@huggingface/transformers` as a dep, make `scripts/build.ts` bundle it (ONNX-WASM, likely an inline worker — the native-binary minefield the original §8.3 anticipated), then **verify model download + embedding in a live Electron vault** (can't be checked headless). |
| **Other semantic backends** (qmd, hosted) | `search/search-backend-factory.ts` (+ new `*-search-backend.ts`) | `QMDSidecarSearchBackend`, `HostedEmbeddingSearchBackend` — each a drop-in `SearchBackend` fused via the existing `rrf.ts`. Recipe in §8.4–8.5. **Honor the no-mandatory-download non-goal** — keep lexical the default. |
...
| **Trusted conformance**                             | new `catalog/trust-manifest.ts`; entry `trustManifest` field exists in `types/ard.types.ts`                       | JWS signing, SPIFFE/DID identity, attestations, provenance. §18 "Future Work".                                                                                                                                                                                                                                                           |
...
**Status: ◐ Partial — hybrid core done, runtime pending (2026-06-23).** The pluggable seam shipped first: `search-backend-factory.createSearchBackend(config)`, wired so a backend-kind change restarts the registry. The **hybrid semantic core is now built and tested** (TDD, 23 new specs):
...
- `search/embedding/transformers-embedder.ts` — real `Embedder` over `@huggingface/transformers`, lazy-imported via a **runtime-computed specifier** so Bun leaves it unbundled (verified: `dist/main.js` stays 1.6 MB, the import is preserved as `import(["@huggingface","transformers"].join("/"))`). The factory routes `local-model` → `SemanticSearchBackend(new TransformersEmbedder(...))`.

So `local-model` is wired end-to-end and safe, but **behaves as lexical until the library is actually bundled** (the lazy import throws `MODULE_NOT_FOUND` → caught → lexical fallback). Remaining work is the runtime, per §1b: add the dep, make `scripts/build.ts` bundle the ONNX-WASM stack (the inline-worker minefield below), and verify model download + embedding in a live Electron vault. qmd/hosted backends are still deferred. The design below stands as the recipe.
