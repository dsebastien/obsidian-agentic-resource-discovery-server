...
| --------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Semantic search — live smoke test** | `search/embedding/http-embedder.ts` | **Done & tested** (see §8.3 status): `local-model` routes to `SemanticSearchBackend` (lexical BM25 ⊕ dense-vector cosine via RRF) over an injectable `Embedder` seam, fed by `HttpEmbedder` against a local OpenAI-compatible `/v1/embeddings` server (Ollama / LM Studio / llama.cpp / LocalAI). Zero bundle, zero managed download. Remaining: **run it against a real server** — start Ollama (`ollama pull nomic-embed-text`), select the backend, point the URL at `http://localhost:11434/v1`, Rescan, and confirm search still ranks (and that a stopped server cleanly falls back to lexical). No server was running in the build env, so this is unverified e2e; the request/response contract is covered by `http-embedder.spec.ts`. |
| **Other semantic backends** (qmd, hosted) | `search/search-backend-factory.ts` (+ new `*-search-backend.ts`) | `QMDSidecarSearchBackend`, `HostedEmbeddingSearchBackend` — each a drop-in `SearchBackend` fused via the existing `rrf.ts`. (`hosted-api` is now largely a remote-URL variant of `HttpEmbedder`.) Recipe in §8.4–8.5. **Honor the no-mandatory-download non-goal** — keep lexical the default. |
